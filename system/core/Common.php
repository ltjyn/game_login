...
if(!function_exists('returnResult')){
	function returnResult($type = '', $data, $template_file = FALSE) {
		$result = array();
		if($template_file){
...
/**
 * 注意 ：这个是异常捕获，下面那个是错误捕获
 */
function exception_handler($e){
    $_error = load_class('Exceptions', 'core');

    if(($e->getCode() & error_reporting()) == $e->getCode()){
        $_error->show_php_error($e->getCode(), $e->getMessage(), $e->getFile(), $e->getLine());
    }

    $_error->log_exception($e->getCode(), $e->getMessage(), $e->getFile(), $e->getLine());
}
...
if(!function_exists('_exception_handler')){
	function _exception_handler($errno, $errstr, $errfile, $errline) {
		...
		if (strpos($errstr, 'Connection refused') !== false) {
            throw new SocketConnectException($errstr,$errno);
		}else{
        ...
        $_error->log_exception($errno, $errstr, $errfile, $errline);
